...
use Hanafalah\ModulePayment\Contracts\Schemas\Voucher as ContractsVoucher;
...
class Voucher extends PackageManagement implements ContractsVoucher
{
    ...
    public function prepareStoreVoucher(?array $attributes = null): Model
    {
        ...
        if (isset($attributes['author_id'])) {
            $author = app(config('module-payment.voucher.author'))->findOrFail($attributes['author_id']);
            $attributes['author_type'] = $author->getMorphClass();
        }

        $benefit_targets = config('module-payment.voucher.benefit_targets');
